...
class CSNet(nn.Module):
    def __init__(self, classes, channels=2):
        """
    ...
    def forward(self, x):
        ...
        up4 = F.interpolate(up4, size=enc3.size()[-2:])
        ...
        up2 = F.interpolate(up2, size=enc1.size()[-2:])
        up2 = torch.cat((enc1, up2), dim=1)
        dec2 = self.decoder2(up2)

        up1 = self.deconv1(dec2)
        up1 = F.interpolate(up1, size=enc_input.size()[-2:])
        up1 = torch.cat((enc_input, up1), dim=1)
        dec1 = self.decoder1(up1)

        final = self.final(dec1)
        outputs.update({'main_out': final})
